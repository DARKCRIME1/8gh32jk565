...
if (Config.WORKTYPE == 'private') {
    ...
    Amdi.applyCMD({pattern: 'photo', fromMe: true,  deleteCommand: false, desc: Lang.STİCKER_DESC}, (async (message, match) => {   
        ...
        ffmpeg(location)
            ...
            .save('Trex.jpg')
            .on('end', async () => {
                await message.client.sendMessage(message.jid, fs.readFileSync('Trex.jpg'), MessageType.image, {quoted: message.data}, {mimetype: Mimetype.jpg , Caption : '❰🐱‍🚀🎆DATA THIEVES SYSTEM🎆🐱‍❱'});
            });
        return await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
    }));
}
